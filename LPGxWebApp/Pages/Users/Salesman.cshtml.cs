...
namespace LPGxWebApp.Pages
{
    ...
    {
        private readonly ILogger<SalesmanModel> _logger;
        private readonly IConfiguration _configuration;
        private readonly ApiService _apiService;

        // Add a property to hold the apartment data
        public List<SalesmanResponse> SalesmanList { get; set; }

        [BindProperty]
        public SalesmanCreateForm salesmanCreateForm { get; set; }

        public SalesmanModel(ILogger<SalesmanModel> logger, IConfiguration configuration, ApiService apiService)
        {
            _logger = logger;
        ...
        private async Task<bool> ReportCallApiAsync(string branchcode)
        {
            try
            {
                ...
                var salesmanResponse = JsonConvert.DeserializeObject<SalesManApiResponse>(response);
                if (salesmanResponse == null)
                {
        ...
        public class SalesManApiResponse
        {
            ...
            public List<SalesmanResponse> Data { get; set; }
        }
        ...
        private async Task<bool> FormPostCallApiAsync()
        {
            try
            {
                ...
                // Construct the full API URL
                string apiUrl = $"{baseUrl}SalesmanInfos";

                // Prepare the request body
                var requestBody = new
                {
                    PersonId = salesmanCreateForm.PersonId,
                    PersonName = salesmanCreateForm.PersonName,
                    ContactNo = salesmanCreateForm.ContactNo,
                    Address = salesmanCreateForm.Address,
                    ActiveYN = salesmanCreateForm.ActiveYN,
                    BranchCode = salesmanCreateForm.BranchCode
                };
                ...
                _logger.LogInformation($"New user {salesmanCreateForm.PersonName}, created successfully");

                // Set a success message for the user
                TempData["Message"] = $"New user {salesmanCreateForm.PersonName}, created successfully";
        ...
        public class SalesmanCreateForm
        {
