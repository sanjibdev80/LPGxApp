...
namespace LPGxWebApp.Pages
{
    public class ReturnModel : PageModel
    {
        ...
        // Add a property to hold the Return data
        public List<ReturnReportResponse> Returns { get; set; }
        ...
        public ReturnModel(ILogger<ReturnModel> logger, IConfiguration configuration, ApiService apiService)
        {
            _logger = logger;
        ...
        private async Task<bool> ReportCallApiAsync(string BranchCode, string SignonName)
        {
            try
            {
                ...
                string apiUrl = $"{baseUrl}ReturnInfos/Date/{BranchCode}/{currentDate}";
                ...
                _logger.LogInformation($"Return data successfully retrieved");
                return true;
            }
            catch (Exception ex)
            {
        ...
        // Consolidated method to handle API calls
        private async Task<bool> ProductCallApiAsync(string BranchCode, string SignonName)
        {
            try
            {
                // Get the API BaseUrl from the configuration
        ...
        private async Task<bool> FormPostCallApiAsync()
        {
            try
            {
                ...
                // Construct the full API URL
                string apiUrl = $"{baseUrl}ReturnInfos";

                // Prepare the request body
                var requestBody = new
                {
                    ReturnSaleId = ProductsForm.ReturnSaleId,
                ...
                var Returnname = data["returnSaleId"]?.ToString();
                if (string.IsNullOrEmpty(Returnname))
                {
                    // Log and notify if "Returnname" is missing or empty
                    _logger.LogError("Response contains no 'Returnname' field.");
                    TempData["Message"] = "No 'Returnname' field in the API response.";
                    TempData["MessageType"] = "Error";
                    return false;
                }

                // Log the successful response with the Building code
                _logger.LogInformation($"TrackId: {Returnname}. Data registered successfully");

                // Set a success message for the user
                TempData["Message"] = $"TrackId: {Returnname}. Data registered successfully";
        ...
        public async Task<IActionResult> OnGetAsync()
        {
            ...
            bool successAptNoList = await ProductCallApiAsync(verifyLoginData.BranchCode, verifyLoginData.SignonName);
            if (!successAptNoList)
            {
